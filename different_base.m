...
for model_flag = 1:4
    ...
    time_proposed_1 = tic;
    [MDl_base, save_name] = choose_mode(model_flag, Input_base, Output_base);
    ...
    for i = 1:size(list, 1)
        ...
        Estimation_base = get_base_output(MDl_base, selected_HI, model_flag);
    ...
    for i = 1:size(list, 1)
        ...
        for j = 2:size(cells, 1)
            ...
            Q_base = get_base_output(MDl_base, HI, model_flag);
    ...
    save(strcat(".\data\", save_name), "Estimation_results_MIT", "Estimation_results_oxford", "Estimation_results_CALCE")
end
function [MDl_base, save_name] = choose_mode(model_flag, Input_base, Output_base)
switch model_flag
    case 1
        MDl_base = polyfit(Input_base, Output_base, 1); %
        save_name = 'Estimation_results_poly1.mat';
    case 2
        MDl_base = polyfit(Input_base, Output_base, 2); %
        save_name = 'Estimation_results_poly2.mat';
    case 3
        MDl_base = polyfit(Input_base, Output_base, 3); %
        save_name = 'Estimation_results_poly3.mat';
    case 4
        save_name = 'Estimation_results_svm.mat';
        MDl_base = fitrsvm(Input_base, Output_base); %
end
end
function output = get_base_output(model, data, model_flag)
if model_flag ~= 4
    output = polyval(model, data);
else
